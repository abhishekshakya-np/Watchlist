...
export default function BookmarkDirectoryCard({ bookmark, onEdit, onDelete, canManage = true }) {
  ...
  const tipId = `bookmark-desc-${bookmark.id}`;
  const hasDesc = Boolean(String(blurb || '').trim());

  return (
    <article className="bookmark-dir-card bookmark-dir-card--directory">
      <a
        href={bookmark.url}
        className="bookmark-dir-card__link bookmark-dir-card__link--directory"
        target="_blank"
        rel="noopener noreferrer"
        aria-describedby={hasDesc ? tipId : undefined}
      >
        ...
        <div className="bookmark-dir-card__body">
          <span className="bookmark-dir-card__title bookmark-dir-card__title--directory">{displayTitle}</span>
          {hasDesc ? (
            <>
              <span id={tipId} className="visually-hidden">
                {blurb}
              </span>
              <p className="bookmark-dir-card__excerpt-touch" aria-hidden="true">
                {blurb}
              </p>
              <div className="bookmark-dir-card__tooltip" aria-hidden="true">
                {blurb}
              </div>
            </>
          ) : null}
